...
function Wo_SendSMSMessage($to, $message) {
    ...
    if ($wo["config"]["sms_provider"] == "zender" && !empty($wo["config"]["zender_siteurl"]) && !empty($wo["config"]["zender_apikey"])) {
        $to = Wo_Secure($to);

        $response = false;

        try {
            if(empty($wo["config"]["zender_service"]) || $wo["config"]["zender_service"] < 2):
                if(!empty($wo["config"]["zender_device"])):
                    $mode = "devices";
                else:
                    $mode = "credits";
                endif;
                ...
                if($mode == "devices"):
                    $params = [
                        "secret" => $wo["config"]["zender_apikey"],
                        "mode" => "devices",
                        "device" => $wo["config"]["zender_device"],
                        "phone" => $to,
                        "message" => $message,
                        "sim" => $wo["config"]["zender_slot"] < 2 ? 1 : 2
                    ];
                else:
                    $params = [
                        "secret" => $wo["config"]["zender_apikey"],
                        "mode" => "credits",
                        "gateway" => $wo["config"]["zender_gateway"],
                        "phone" => $to,
                        "message" => $message
                    ];
                endif;

                $apiurl = "{$wo["config"]["zender_siteurl"]}/api/send/sms";
            else:
                $params = [
                    "secret" => $wo["config"]["zender_apikey"],
                    "account" => $wo["config"]["zender_whatsapp"],
                    "type" => "text",
                    "recipient" => $to,
                    "message" => $message
                ];

                $apiurl = "{$wo["config"]["zender_siteurl"]}/api/send/whatsapp";
            endif;
        ...
        return $response;
    } elseif ($wo["config"]["sms_provider"] == "twilio" && !empty($wo["config"]["sms_twilio_username"]) && !empty($wo["config"]["sms_twilio_password"]) && !empty($wo["config"]["sms_t_phone_number"])) {
